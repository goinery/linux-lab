...
QT\_QPA\_PLATFORM=offscreen ./build/bin/chatroom --help
...
s=socket.create\_connection(('127.0.0.1',8888),timeout=timeout)
...
print('LOGIN\_OK', send({'type':'login','username':u,'password':p}))
print('LOGIN\_BAD', send({'type':'login','username':u,'password':'badpass'}))
PY

预期：

- REGISTER 返回 register\_response 且 success 为 true。
- LOGIN\_OK 返回 login\_response 且 success 为 true。
- LOGIN\_BAD 返回 login\_response 且 success 为 false。
